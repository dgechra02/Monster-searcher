...
apiUser
  ...
  .then((data) => {
    ...
    searchInput.addEventListener("input", function () {
      const cards = document.getElementById("cards");
      count = 0; // making it zero on each render
      ...
      userArray
        .filter((element) => {
          if (
            ...
            element.username.toLowerCase().includes(searchInput.value.toLowerCase()) ||
            element.email.toLowerCase().includes(searchInput.value.toLowerCase())
          )
            return element;
          else count++;
      ...
      if (count === 10) {
        errorMessage.textContent = "No monster found! search another!!";
